...
class NA_CleanAmmo extends RecipeBase
{
	void NA_CleanAmmo()
	{
	...
	override bool CanDo(ItemBase ingredients[], PlayerBase player)//final check for recipe's validity
	{
		...
		return module_cleaning.CanRepair(ingredient1,ingredient2);
	}
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		ItemBase ingredient2;
		Class.CastTo(ingredient2, ingredients[1]);
		module_cleaning.Repair(playerPB, ingredient1,ingredient2,m_Specialty);
	}
};
