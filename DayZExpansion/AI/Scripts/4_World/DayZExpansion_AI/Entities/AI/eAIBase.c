...
class eAIBase: PlayerBase
{
	...
	bool PlayerIsEnemy(EntityAI other, bool track = false, out bool isPlayerMoving = false)
	{
		...
		if (player.GetGroup())
		{
			...
			if (player.GetGroup().GetFaction().IsFriendly(GetGroup().GetFaction()) || player.GetGroup().GetFaction().IsFriendly(this, player))
			{
#ifdef DIAG
				if (eAI_IsPassive())
		...
		if (GetGroup().GetFaction().IsFriendly(other, this))
		{
#ifdef DIAG
			if (eAI_IsPassive())
	...
	void UpdateTargets(float pDt)
	{
		...
		while (m_eAI_CurrentPotentialTargetIndex < m_eAI_PotentialTargetEntities.Count())
		{
			...
			else if (Class.CastTo(targetItem, obj))
			{
				...
				continue;
			else if (Class.CastTo(targetEntity, obj) && GetGroup().GetFaction().IsFriendly(targetEntity, this))
				continue;
