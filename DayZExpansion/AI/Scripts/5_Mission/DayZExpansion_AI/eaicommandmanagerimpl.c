//! This class handles the inputs from the eAICommandMenu
class eAICommandManagerImpl: eAICommandManager
{
		...
		switch (cmd)
		{
			...
			default:
				...
				else if (cmd - eAICommands.BEH_LOOT >= 0)
				{
					...
					if (lootingBehavior == eAILootingBehavior.NONE)
						m_LootingBehavior = eAILootingBehavior.NONE;
					else if ((m_LootingBehavior & lootingBehavior) == lootingBehavior)
	...
	void RPC_SetLootingBehavior(PlayerIdentity sender, Object target, ParamsReadContext ctx)
	{
		...
		if (g)
		{
			...
			if (bitmask > -1)
			{
				...
				if (lootingBehavior && (bitmask & lootingBehavior) == bitmask)
				{
