...
class ExpansionActionDamageBaseBuilding: ActionContinuousBase
{
	...
	{	
		...
		if ( target_obj.IsKindOf("ExpansionBaseBuilding") && item.IsMeleeWeapon() && GetExpansionSettings().GetRaid(false).IsLoaded() && GetExpansionSettings().GetRaid().IsRaidableNow())
		{
			return true;
		}
