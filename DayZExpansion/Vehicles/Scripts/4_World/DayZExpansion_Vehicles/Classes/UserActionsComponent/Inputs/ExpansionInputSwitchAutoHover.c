/*
 * ExpansionInputSwitchAutoHover.c
 *
 * DayZ Expansion Mod
...
class ExpansionInputSwitchAutoHover : DefaultActionInput
{
	ref ActionTarget target_new;

	void ExpansionInputSwitchAutoHover( PlayerBase player )
	{
		SetInput( "UAExpansionHeliAutoHover" );
	...
	override void UpdatePossibleActions( PlayerBase player, ActionTarget target, ItemBase item, int action_condition_mask )
	{
		...
		if ( player && player.IsInVehicle() && player.GetParent() )
		{
			...
			{
				target_new = new ActionTarget( Object.Cast( player.GetParent() ), NULL, -1, vector.Zero, -1 );
