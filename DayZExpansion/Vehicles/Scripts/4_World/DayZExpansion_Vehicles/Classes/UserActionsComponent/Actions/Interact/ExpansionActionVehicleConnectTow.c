...
[RegisterAction(ExpansionActionVehicleConnectTow)]
#endif
class ExpansionActionVehicleConnectTow : ActionInteractBase
{
	...
	override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item)
	{
		...
		if (!GetExpansionSettings().GetVehicle(false).IsLoaded() || GetExpansionSettings() && !GetExpansionSettings().GetVehicle().Towing)
			return false;
