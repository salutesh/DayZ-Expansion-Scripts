...
*/
	...
	void PerformGetOutTransport(ActionData action_data)
	{
		...
		if (vehCommand)
		{
			ExpansionVehicleBase veh = vehCommand.GetTransport();

			float speed = veh.GetSpeedometer();
			bool keepInVehicleSpaceAfterLeave = s_ExpansionPlayerAttachment && veh.Expansion_CanObjectAttach(action_data.m_Player) && veh.LeavingSeatDoesAttachment(vehCommand.GetVehicleSeat());
