...
class ExpansionPersonalStorageModule: CF_ModuleWorld
{
	...
	static string s_PersonalStorageDataFolderPath = "$mission:expansion\\personalstorage\\";
	...
	{
		if (!FileExist(s_PersonalStorageDataFolderPath))
			ExpansionStatic.MakeDirectoryRecursive(s_PersonalStorageDataFolderPath);
	}
	...
	protected void LoadPersonalStorageServerData()
	{
		...
		array<string> personalStorageFiles = ExpansionStatic.FindFilesInLocation(s_PersonalStorageDataFolderPath, ".json");
		if (personalStorageFiles && personalStorageFiles.Count() > 0)
		{
			foreach (string fileName: personalStorageFiles)
			{
				GetPersonalStorageData(fileName, s_PersonalStorageDataFolderPath);
			}
		}
		else
		{
	...
	protected void CreateDefaultPersonalStorageData()
	{
		...
		ExpansionPersonalStorageData personalStorage = new ExpansionPersonalStorageData();
	...
	protected void LoadPersonalStorageItemData(string playerUID)
	{
		...
		string storagePath = ExpansionPersonalStorageModule.s_PersonalStorageDataFolderPath + playerUID + "\\";
	...
	protected void RemoveItemByGlobalID(string playerUID, TIntArray globalID)
	{
		...
		string filePath = s_PersonalStorageDataFolderPath + playerUID + "\\" + fileName + ".json";
