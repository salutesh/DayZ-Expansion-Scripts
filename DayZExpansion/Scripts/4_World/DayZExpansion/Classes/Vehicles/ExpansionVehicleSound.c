class ExpansionVehicleSound
{
	...
	void ExpansionVehicleSound( ExpansionVehicleScript vehicle, string soundSetName, ref array< string > variables )
	{
		m_Vehicle = vehicle;
	...
	bool Update( float pDt, ref array< float > variables )
	{
		if ( !m_SoundWaveObject )
		{
			...
			{
				Error( "[Error][Sound]: ExpansionVehicleSound::OnUpdate() -> m_SoundObject is null -> m_SoundSetName: " + m_SoundSetName );
